...
for i in range(radius_discrete.shape[0]):
    ...
    Signal_MC_protocol  = np.loadtxt('Data/MC_simulations_Connectome2_waveforms_6bvals/simulationsOuputsD08_G500_WF/myelin_' + str(radius_i) + '0_um_D_0.8_2D_G500_WF_DWI.txt')
...
# ---------------------------- Load MC simulations -----------------------------
# --------------------------------- SPIRAL ----------------------------------- #
Signal_spiral = np.loadtxt('Data/MC_simulations_Connectome2_waveforms_6bvals/Spiral/simulations_ouputs_myelin_spiral_model_D08_WF/myelin_0.70_um_D_0.8_2D_spiral_wraps_rep_00_DWI.txt')
...
#concentric, = plt.plot(b_values, S_MC_averague, marker='s', color='black', mfc='none', linewidth=0, label=r"MC signal: Multiple concetric cylinders (Eq.[19])")
...
# Create new area for the zoom plot
zoom_ax = plt.axes([0.595, 0.62, 0.3, 0.25])  # Cambia el valor de 'left' para mover a la derecha

# Add lines and points
zoom_ax.plot(b_values, Signal_06_anat, color=colors[0], linewidth=1)
zoom_ax.plot(b_values, Signal_07_anat, color=colors[1], linewidth=1)
zoom_ax.plot(b_values, Signal_08_anat, color=colors[2], linewidth=1)
zoom_ax.plot(b_values, Signal_09_anat, color=colors[3], linewidth=1)

# Add points
zoom_ax.plot(b_values, S_MC_06, marker='o', color=colors[0], linewidth=0)
zoom_ax.plot(b_values, S_MC_07, marker='o', color=colors[1], linewidth=0)
zoom_ax.plot(b_values, S_MC_08, marker='o', color=colors[2], linewidth=0)
zoom_ax.plot(b_values, S_MC_09, marker='o', color=colors[3], linewidth=0)

zoom_ax.plot(b_values, S_MC_spiral, marker='x', color='black', mfc='none', linewidth=0)
#zoom_ax.plot(b_values, S_MC_averague, marker='s', color='black', mfc='none', linewidth=0)
zoom_ax.plot(b_values, S_MC_averague, marker='s', color='cyan', mfc='none', linewidth=0)


# Configuration: zoom
zoom_ax.set_xlim([1.95, 3.05])
zoom_ax.set_ylim([0.5, 0.66])
zoom_ax.set_xticks([2.0, 2.5, 3.0])
zoom_ax.set_yticks([0.5, 0.55, 0.6, 0.65])

# Configuration: zoom
zoom_ax.tick_params(axis='both', which='major', labelsize=10)

rect = plt.Rectangle((1.95, 0.5),   # Coordinates (x, y) of the rectangle
                     1.1,           # with (in x)
                     0.165,         # height(in y)
                     linewidth=1.2,
                     edgecolor='gray',
...
plt.savefig('Fig5B_Radius_SMT_Anat_MC_Delta_single_axon_multiple_layers_zoom' + '_D_'  + str(D) + '_inner_radius' + str(r_i) + '.png', bbox_inches='tight', dpi=300)
